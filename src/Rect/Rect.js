...
export default class Rect extends PureComponent {
  ...
  startRotate = (e) => {
    ...
    const onMove = (e) => {
      ...
      this.setState(state => ({...state, angle}))
  ...
  handleMouseMove = (e) => {
    ...
    let distanceFormLeft = null;
    const angle = this.state.angle
    if (angle > 225 && angle <= 315) {
      distanceFormLeft = e.clientY - e.currentTarget.offsetLeft
      console.log('merde')
    } else if (angle > 315 || angle <= 45) {
      distanceFormLeft = e.clientX - e.currentTarget.offsetLeft
    } else if (angle > 45 && angle <= 135) {
      distanceFormLeft = e.clientY + e.currentTarget.offsetLeft
    } else if (angle > 135 && angle <= 225) {
      distanceFormLeft = e.clientX - e.currentTarget.width - e.currentTarget.offsetLeft
    }
    this.setState( state => ({...state,
      precisionBarX: distanceFormLeft > totalWidth - 2 ? totalWidth - 2 : distanceFormLeft
    }));
  }
  ...
  render () {
    ...
    return (
      ...
      >
        <div className="precisionBar" style={ {transform: `translate(${this.state.precisionBarX}px, 0)`} }/>
        {
          rotatable &&
